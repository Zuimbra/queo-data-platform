...
def test_lakehouse_directories_are_derived_from_data_dir(
    ...
    assert settings.lakehouse_dir == (
        tmp_path.resolve() / "lakehouse"
    )

    assert settings.control_dir == (
        tmp_path.resolve() / "lakehouse" / "00_control"
    )

    assert settings.bronze_dir == (
        tmp_path.resolve() / "lakehouse" / "01_bronze"
    )

    assert settings.silver_dir == (
        tmp_path.resolve() / "lakehouse" / "02_silver"
    )

    assert settings.gold_dir == (
        tmp_path.resolve() / "lakehouse" / "03_gold"
    )